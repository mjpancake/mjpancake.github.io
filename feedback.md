...
- 在[发布贴]({{ site.data.link.tie }}){:target="_blank"}回复即可
    - 因新道寺中坚导致无法使用贴吧的，可在
      [GitHub issues]({{ site.data.link.issues }}){:target="_blank"}
      发起新issue
...
1. 反馈渠道从发布贴和GitHub中二选一
    1. 通过其它渠道反馈的（尤其是非公开渠道）无法结算贡献度
2. 一条反馈对应一个楼层（或issue），以方便分别讨论
    1. 若有多条反馈，拆分成多层回复，不要挤进同一楼层
3. 反馈须提供有用的新信息
    1. 除非能提供新情报，不要重复发表别人已发过的内容
        1. ”别人是否发过“的判断标准是是否已在最新发布贴出现
    1. 除非能提供新情报，不要发表已在[众包平台](/crowd/)上开始实施的内容
    1. 有捣乱倾向，内容毫无营养的，无法获得贡献度
4. 如果是有关牌局的，附上牌谱截图
...
1. 按照下面的「模板」格式填写
    1. 将模板*复·制·粘·贴*，并替换尖括号部分
        1. 尖括号部分以外*一·个·字·也·不·能·改*
        1. 尖括号替换部分以内可换行，可附图
