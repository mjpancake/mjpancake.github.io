...
毫兔的来源说来话长，
有时间的话可阅读[「双空间混合牌山系统」](/docs/libsaki/mount/)一文。
...
一共插了四处。
假如一切正常，自己摸牌前应该输出 A, C, D；
而他家输出前应该输出 A, B。

三狗测试，每家摸牌前都输出了 A, B。
这个结果意味着：

1. 能看到输出 A，代表`ondraw`的确被执行了，问题出在`ondraw`内部；
2. 无论是自己和他家都输出 B，代表`if`条件并没能区分自家与他家。
